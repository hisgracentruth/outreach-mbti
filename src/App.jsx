...
const OutreachMBTIApp = () => {
  ...
  const results = {
    ...
    'DSIB': { 
      nickname: '지혜로운 부엉이', 
      emoji: '🦉', 
      description: '지혜로운 부엉이형인 당신은 언제나 조용하지만 빛나는 통찰력으로 팀을 든든하게 받쳐주는 숨은 공로자예요. 밤늦게까지도 자료를 찾아 공부하고, 한 번 맡은 일은 철저하게 준비해두는 성실함이 돋보여요. 회의 중에는 앞에 나서서 큰소리치기보다는, 필요한 순간에 꼭 맞는 조언이나 정보를 건네 주변을 깜짝 놀라게 하곤 해요. 겉으로 튀지는 않아도 그 깊이 있는 한 마디 말에 모두가 고개를 끄덕이는 신뢰받는 조력자예요. 이 유형은 개인지향적 성향이 강해서 팀원 한 사람 한 사람과의 소통을 특히 중시해요. 당신은 단체 메시지보다는 개인적으로 다가가 조언하거나 격려하는 방식을 선호하죠. 예를 들어 새로 온 팀원이 어색해하면 살짝 다가가 말을 걸고, 어려움을 겪는 동역자가 있으면 조용히 커피 한 잔 건네며 위로를 전해요. 이렇게 동행하는 태도 덕분에 주변 사람들은 당신에게 마음을 터놓기 쉽고, 은근한 힘을 얻곤 해요. 선언형 기질을 지니고 있지만 앞에 나서는 리더라기보다는 꼭 필요한 때에 진리를 일깨워주는 안내자 역할을 잘해요. 회의나 나눔 시간에 당신이 가끔 조용히 입을 열면 모두 경청하는 분위기가 될 정도로 말에 힘이 실려 있어요. 당신은 말할 가치가 있는 내용만 말하기 때문에 그 통찰이 더욱 돋보이고, 팀원들은 “역시 생각을 많이 해본 얘기야”라는 반응을 보이곤 해요. 언뜻 수수해 보이지만 울림 있는 소통 방식이 부엉이형의 매력이에요. 구조형답게 체계와 원칙을 중요하게 여겨 맡은 일은 차질 없이 해내려고 애써요. 회의록 정리부터 자료 아카이브까지 꼼꼼히 챙기고, 스케줄이나 계획표를 깔끔하게 관리하는 모습에서 당신의 프로다운 면모가 드러나요. 앞에 나서지는 않아도 마음속에는 “이 방향대로만 가면 좋을 텐데...” 하고 팀의 진행을 걱정하기도 해요. 그래서 리더나 팀이 헤매는 듯할 때 슬쩍 방향을 잡아주는 조언을 건네며, 자연스럽게 팀의 나침반 역할을 하곤 해요. DSIB형인 당신은 눈에 띄지 않는 곳에서도 지혜와 성실로 팀을 세워주는 귀한 존재예요. 본인은 조연이라고 생각할지 몰라도, 당신의 조언과 정리 덕분에 사역이 한층 견고해져요. 사람을 향한 따뜻한 관심과 체계적인 지원으로 하나님께서 맡기신 영혼들을 든든하게 뒷받침하는 모습이 참 아름다워요. 앞으로도 그 지혜의 등불을 밝게 비추어서 많은 이들이 길을 찾는 데 도움을 주길 기대해요.',
      strengths: [
